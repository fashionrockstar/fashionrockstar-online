...
Project pages use the existing clean UI sans-serif family, compact headings, left-aligned role information, the year on the right, and more space between media. The owner approved these project interiors. Following review, Work was restored exactly to its pre-layout-update markup from commit 61eb86e: full-width covers in their supplied proportions, original title placement and desktop hover/touch visibility. Work no longer loads project-layout.css. All ten project links, their ordering, galleries and the Micaela looping video remain intact. Source and playback-controller checks pass; a fresh automated visual check is unavailable because the browser connection cannot start.
